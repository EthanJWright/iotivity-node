...
function bindStringsToResource( handle, strings, binder, reject, doDelete ) {
	var index, result, errorMessage;
	var extension = {};

	for ( index = 0; index < strings.length; index++ ) {
		result = csdk[ binder ]( handle, strings[ index ] );
...
_.extend( Server.prototype, {
	...
	_createEntityHandler: function( resource ) {
		return _.bind( function( flag, request, url ) {
			...
			eventName =
				...
				request.method === csdk.OCMethod.OC_REST_PUT ?
					( request.payload &&
				...
				request.method === csdk.OCMethod.OC_REST_POST && !resource && url ? "create" :
				undefined;
			...
			if ( eventName in { "retrieve": 0, "update": 0, "delete": 0 } && resource ) {
				...
				if ( create ) {
					...
					csdk.OCDoResponse( _.extend( {
						requestHandle: request.requestHandle,
						ehResult: csdk.OCEntityHandlerResult.OC_EH_RESOURCE_NOT_FOUND,
						sendVendorSpecificHeaderOption: [],
						resourceUri: resource ? resource.resourcePath : url,
						sendVendorSpecificHeaderOptions: [],
						payload: null
					}, resource ? { resourceHandle: resource._private.handle } : {} ) );
				}
			}
	...
	register: function( init ) {
		...
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			if ( !( bindStringsToResource( resource._private.handle, init.resourceTypes.slice( 1 ),
						"OCBindResourceTypeToResource", reject, true ) &&
					bindStringsToResource( resource._private.handle, init.interfaces.slice( 1 ),
						"OCBindResourceInterfaceToResource", reject, true ) ) ) {
				return;
			}
